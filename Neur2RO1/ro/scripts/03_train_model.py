...
# import pandas as pd
# import pickle as pkl
# import os  # <<< INSERTED: for path join + mkdir
...
#     # >>> INSERTED START: force offering_no_network to save under data/offering_no_network/random_search/
#     # and ensure result filename matches 04_get_best_model scan pattern (set_encoder_tr_res*.pkl).
#     if args.problem == "offering_no_network":
#         data_root = str(cfg.data_path).strip()
#         if data_root.startswith("./"):
#             data_root = data_root[2:]
#         data_root = data_root.rstrip("/")

#         rs_dir = os.path.join(data_root, "offering_no_network", "random_search")
#         os.makedirs(rs_dir, exist_ok=True)

#         # fp_res must end with ".pkl" and start with "{model_type}_tr_res"
#         fp_res = os.path.join(rs_dir, f"{args.model_type}_tr_res.pkl")

#         # fp_model must end with ".pt" and start with "{model_type}"
#         fp_model = os.path.join(rs_dir, f"{args.model_type}.pt")
#     # <<< INSERTED END
